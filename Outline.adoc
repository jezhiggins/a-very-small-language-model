...
Well, it's really quite simple. We just look at each word in turn, and see what comes after it. Now some following words will occur more than once, so we keep a running count of those occurrances. Once we've read our whole corpus, we will have a list of all the words, and which words come after them.
